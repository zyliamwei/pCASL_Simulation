...
fovsatgradreph=-0.14*74;
% Magnetic-field inhomogeneity
maginhomo=0.07*74;
fovsatgrad=fovsatgrad+maginhomo; 
fovsatgradreph=fovsatgradreph+maginhomo;
...
for ni=1:labnum
    ...
    % Polarity of labeling pulse (labeled/control)
    if mod(ni,2)==1
        label_polar=[label_polar zeros(size(labmod_rf))];
    else
        label_polar=[label_polar ones(size(labmod_rf))];
    end
end
figure;subplot(3,1,1);
plot(label_rf,'r');title('Labeling pulse train');
ylabel('Normalized amplitude');
subplot(3,1,2);plot(label_grad,'b');title('Labeling gradient train');
ylabel('Gradient (G/cm)');
subplot(3,1,3);plot(label_polar,'m');title('Label pulse polarity');
ylabel('Polarity');
...
for ni=1:length(label_rf)
    ...
    if label_polar(ni)==1
        sig(:,ni)=ZRot(label_angle(ni),90)*sig(:,ni);
    else
        sig(:,ni)=ZRot(label_angle(ni),270)*sig(:,ni);
    end
    sig(:,ni)=tempA*sig(:,ni)+tempB;    
end
...
figure;plot(label_z,sig(3,:),'r');
xlabel('Position (cm)');
ylabel('Normalized magnetization');
axis([z(1)/10 z(2)/10 -1 1]);
W_Plot;
